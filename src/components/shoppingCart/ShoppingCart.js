...
export default function ShoppingCart() {
  ...
  useEffect(() => {
    const cartData = JSON.parse(localStorage.getItem("courseShoppingCart"));
    let total = 0
  ...
  const addHandler = (item) => {
    ...
    setCart(newCart)
    localStorage.setItem("courseShoppingCart", JSON.stringify(newCart))
  }
  ...
  return (
    <div className='w-full flex gap-x-2 px-5 py-3'>
      <div className="w-3/4  flex flex-col gap-2">
        <h1 className="font-dana-demiBold mb-2">سبد خرید شما : {(cart.length).toLocaleString("fa-IR")} مرسوله</h1>
        {cart.map(item => <div key={uuid()} className=' w-full border border-green-400 rounded-md p-2'>
          <div className='flex w-full'>
            <div className="w-11/12 flex">
              ...
              <div className="flex flex-col">
                <div className="font-dana-demiBold">{item.name}</div>
                <div className='font-dana'>قیمت : {(item.price).toLocaleString("fa-IR")} تومان</div>
                <div className='font-dana'>تعداد : {(item.count).toLocaleString("fa-IR")} عدد</div>
                <div className="flex">
      ...
      <div className="flex flex-col  gap-2 w-1/4">
        <h1 className="font-dana-demiBold mb-2">مجموع پرداختی شما</h1>
        <div className=" border border-green-500 rounded-md p-4 h-fit">
          <div className='flex flex-col gap-2'>
            <div className="flex flex-col gap-2">
              <div className="flex justify-between font-dana">
                <div>تعداد محصول</div>
            ...
            <button className="bg-green-400 w-full rounded-md font-dana hover:bg-white hover:text-green-400 p-2 hover:border hover:border-green-400 hover:font-dana-demiBold">تایید و تکمیل سفارش</button>
            <div className="font-dana text-sm text-gray-400">
