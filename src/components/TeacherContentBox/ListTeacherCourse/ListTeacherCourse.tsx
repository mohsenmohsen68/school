...
export default function ListTeacherCourses({user}) {
  const dispatch = useDispatch<AppDispatch>();
  const courses = useSelector<RootState>((state) => state.courses.map(item => {
    console.log("teacher : ", item.teacher)
    if(item.teacher._id == user._id){
        return item
    }
  }));
